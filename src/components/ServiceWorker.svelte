<script type="ts">
        ...
        try {
          ...
          setTimeout(async () => {
            if(!updateFound) {
              ...
              installing = false;
              await registerPullNotifications();
            }
          }, 100);
        } catch (err) {
  ...
  // Function to setup pull notifications:
  async function registerPullNotifications() {
    // Register periodic sync if supported:
    const swRegistration = await navigator.serviceWorker.ready;
    if ("periodicSync" in swRegistration) {
      try {
        await (swRegistration.periodicSync as any).register("pull-events", {
      ...
      navigator.permissions.query({ name: "periodic-background-sync" } as any).then(console.log);
      ...
      if (!("Notification" in window)) {
        ...
        Notification.requestPermission().then((permission) => {
          ...
          if (permission === "granted") {
            new Notification("Notifications set for every day!");
          }
        });
      }
    } else {
      alert("Your browser does not support periodic sync...");
    }
  }
